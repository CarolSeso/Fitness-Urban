      ...
      {ecranMare ? (
        <div className="fixed z-10 grid w-full grid-cols-4 p-2 bg-black border-b-2 border-neutral-800 ">
          ...
          <div className="flex items-center col-span-3 mx-14">
            ...
            <Link to="/workouts" className="mx-12 text-3xl text-white transition-all duration-150 hover:text-4xl hover:border-b-4 hover:border-lime-400 hover:text-lime-400">Workouts</Link>
          </div>
          <div className="absolute flex flex-col gap-4 bg-black text- xl:flex-row w-76 top-5 right-5 xl:top-12 xl:right-10">
            <Link to="/logIn">
              <button type="submit" className="w-24 p-1 transition-all duration-75 ease-in bg-black border-2 rounded-full text-lime-400 border-lime-400 hover:text-black hover:font-semibold hover:bg-lime-400 hover:border-lime-400">Log In</button>
            </Link>
            <Link to="/signUp">
              <button type="submit" className="w-24 p-1 font-semibold text-black transition-all duration-75 rounded-full ease-inf bg-lime-400 hover:text-white hover:border-lime-800 hover:bg-lime-800">Sign Up</button>
            </Link>
          </div>
        </div>
      ) : (
        <div className="fixed z-10 flex justify-center w-full bg-black border-b-2 border-neutral-800">
          ...
          {!menuButton ? <HiMenu onClick={changeMenuButton} className="absolute text-white transition-all duration-50 w-14 h-14 right-7 top-7 hover:text-lime-400" />
            : (
            <div className="fixed top-0 right-0 z-10 h-full grid-flow-row gap-10 text-white bg-black border-l-2 border-neutral-800 w-80">
              ...
              <div className="flex items-center mb-3 space-x-3">
                ...
                <a href="https://www.facebook.com/FitnessUrbanRomania">
                  <FaFacebookSquare className="text-blue-800 transition-all duration-200 rounded-md w-7 h-7 hover:bg-white" />
                </a>
                <a href="https://www.youtube.com/@DORUDOROBAT">
                  <FaYoutube className="w-10 text-red-900 transition-all duration-200 rounded-lg h-7 hover:bg-white" />
                </a>
              </div>
              </div>
              <div className="absolute right-0 flex flex-row w-56 gap-3 bg-black top-2">
                <Link to="/logIn">
                  <button type="submit" className="w-20 p-1 transition-all duration-75 ease-in bg-black border-2 rounded-full text-lime-400 border-lime-400 hover:text-black hover:font-semibold hover:bg-lime-400 hover:border-lime-400">Log In</button>
                </Link>
                <Link to="/signUp">
                  <button type="submit" className="w-20 p-1 font-bold text-black transition-all duration-75 border-2 rounded-full ease-inf bg-lime-400 hover:text-white border-lime-400 hover:border-lime-800 hover:bg-lime-800">Sign Up</button>
                </Link>
              </div>
            </div>
            )
